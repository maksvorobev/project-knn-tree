...
public:
    ...
    void start(){
        ...
        std::vector<int> types;
            ...
            for (int i = 0; i < indices.size(); ++i){
                ...
                if (map.count(vec) == 0) assert(false);
                types.push_back(map[vec]);
            }

        std::vector<int> allTypes;
        if (rank == 0) {
            allTypes.resize(numNeighbors * size); // Подготовка места для данных от всех процессов
        }
        check_rc(MPI_Gather(types.data(), numNeighbors, MPI_INT, \
           allTypes.data(), numNeighbors, MPI_INT, \
           0, MPI_COMM_WORLD), "MPI_Gather");
    ...
    Eigen::Map<Eigen::VectorXd>(distancesLinear.data(), numNeighbors) = distances;

    // Создание буфера для сбора данных в процессе 0
    std::vector<double> gatheredDistances;
    if (rank == 0) {
        gatheredDistances.resize(numNeighbors * size); // Подготовка места для данных от всех процессов
    }
    ...
    if (rank == 0) {
            ...
            for (int i = 0; i < numNeighbors*size; ++i) compressAll.push_back(std::pair(gatheredDistances[i], allTypes[i]));
            std::sort(compressAll.begin(), compressAll.end(), [](auto pair1, auto pair2){ return (pair1.first <= pair2.first); });
            ...
            std::cout << "Program predict that truth is " << (zeroes >= units ? 0 : 1) << '\n';
